...
import org.spatialite.CrossProcessCursorWrapper;
import org.spatialite.Cursor;
import org.spatialite.DatabaseUtils;
import org.spatialite.SQLException;
import org.spatialite.database.SQLiteDebug.DbStats;
...
public class SQLiteDatabase extends SQLiteClosable {
	...
	private static final String ICU_DAT = "icudt51l.dat";
	private static final String ICU_ZIP = "icudt51l.zip";
    ...
    private static void loadICUData(Context context, File workingDir, String icuDatFile, String icuZipFile) {

        try {
            File icuDir = new File(workingDir, "icu");
            if(!icuDir.exists()) icuDir.mkdirs();
            File icuDataFile = new File(icuDir, icuDatFile);
            if(!icuDataFile.exists()) {
                ZipInputStream in = new ZipInputStream(context.getAssets().open(icuZipFile));
                in.getNextEntry();
    ...
    public static void loadLibs (Context context) {
        loadLibs(context, context.getFilesDir(), ICU_DAT, ICU_ZIP);
    }

    public static void loadLibs (Context context, File workingDir, String icuDatFile, String icuZipFile)
    {
	    // Experimental stlport_static definition in Application.mk
        //System.loadLibrary("stlport_shared");

        // Iconv is dependency of android_spatialite and on Intel emulator and the dynamic liker
        // fails to load this dependency. So lets load it manually before android_spatialite.
        // The reason of problem is described here: 
        // http://grokbase.com/t/gg/android-ndk/11bjswgrps/loading-library-with-dependencies#20111118fibt5wnvyf2kkjndxflohmrzmi
        //System.loadLibrary("iconv");
        System.loadLibrary("android_spatialite");

	    //boolean systemICUFileExists = new File("/system/usr/icu/icudt46l.dat").exists();
        boolean systemICUFileExists = new File("/system/usr/icu/" + icuDatFile).exists();

        String icuRootPath = systemICUFileExists ? "/system/usr" : workingDir.getAbsolutePath();
        setICURoot(icuRootPath);
        if (!systemICUFileExists){
            loadICUData(context, workingDir, icuDatFile, icuZipFile);
        }
        
        // We target ICS that means there always exist an ICU dat file (4.6 or newer)
        // http://developer.android.com/reference/java/util/Locale.html
        //String icuRootPath = "/system/usr";
        //setICURoot(icuRootPath);
    }
     ...
     */
    /* package */ Map<String, SQLiteCompiledSql> mCompiledQueries = new HashMap<String, SQLiteCompiledSql>(); // SV Maps.newHashMap();
    /**
     * @hide
     */
     ...
     */
    public static SQLiteDatabase openDatabase(String path, CursorFactory factory, int flags, SQLiteDatabaseHook databaseHook) {
        SQLiteDatabase sqliteDatabase = null;
        try {
            // Open the database.
            sqliteDatabase = new SQLiteDatabase(path, factory, flags, databaseHook);
    ...
    public static SQLiteDatabase openDatabase(String path, CursorFactory factory, int flags) {
        // SV return openDatabase(path, factory, CREATE_IF_NECESSARY, null);
    ...
    // SV
    public native void dumpTxtTab(String path, String table, String charset);
