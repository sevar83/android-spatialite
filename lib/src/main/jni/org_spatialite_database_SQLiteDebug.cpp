...
{
    { "getPagerStats", "(Lorg/spatialite/database/SQLiteDebug$PagerStats;)V",
            (void*) getPagerStats },
...
int register_android_database_SQLiteDebug(JNIEnv *env)
{
    ...
    clazz = env->FindClass("spatialite/database/SQLiteDebug$PagerStats");
    if (clazz == NULL) {
        LOGE("Can't find org/spatialite/database/SQLiteDebug$PagerStats");
        return -1;
    }
    ...
    return jniRegisterNativeMethods(env, "org/spatialite/database/SQLiteDebug",
