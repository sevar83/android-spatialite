...
int register_android_database_SQLiteStatement(JNIEnv * env)
{
    ...
    clazz = env->FindClass("org/spatialite/database/SQLiteStatement");
    if (clazz == NULL) {
        LOGE("Can't find org/spatialite/database/SQLiteStatement");
        return -1;
    }
    ...
    return jniRegisterNativeMethods(env, "org/spatialite/database/SQLiteStatement",
