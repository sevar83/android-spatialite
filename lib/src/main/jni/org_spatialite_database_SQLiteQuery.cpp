...
// SV: Attempt to disable the overwhelming LOG_WINDOW logs if cursor window is very big
#define LOG_NDEBUG 1
...
static JNINativeMethod sMethods[] =
{
    ...
    {"native_fill_window", "(Lorg/spatialite/CursorWindow;IIII)I", (void *)native_fill_window},
...
int register_android_database_SQLiteQuery(JNIEnv * env)
{
    ...
    clazz = env->FindClass("org/spatialite/database/SQLiteQuery");
    if (clazz == NULL) {
        LOGE("Can't find org/spatialite/database/SQLiteQuery");
        return -1;
    }
    ...
    return jniRegisterNativeMethods(env,
        "org/spatialite/database/SQLiteQuery", sMethods, NELEM(sMethods));
}
