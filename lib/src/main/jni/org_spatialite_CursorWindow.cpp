    ...
    static void native_init_empty(JNIEnv *env, jobject object, jboolean localOnly) {
        ...
        if (!window) {
            jniThrowException(env, "java/lang/RuntimeException",
                              "No memory for native window object");
            return;
        }
...
#else
                return strtoll((char const *) window->offsetToPtr(field.data.buffer.offset), NULL,
                               0);
 ...
 */
    ...
    static jcharArray copyStringToBuffer_native(JNIEnv *env, jobject object, jint row,
        ...
        if (type == FIELD_TYPE_STRING) {
            ...
            if (size > 0) {
                ...
                jstring content = env->NewString(
                        (const jchar *) window->offsetToPtr(field.data.buffer.offset), length);
    ...
    static jboolean putBlob_native(JNIEnv *env, jobject object, jbyteArray value, jint row,
                                   jint col) {
        CursorWindow *window = GET_WINDOW(env, object);
        if (!value) {
    ...
    static jboolean putString_native(JNIEnv *env, jobject object, jstring value, jint row,
                                     jint col) {
        CursorWindow *window = GET_WINDOW(env, object);
        if (!value) {
            LOG_WINDOW("How did a null value send to here");
    ...
    static jboolean putDouble_native(JNIEnv *env, jobject object, jdouble value, jint row,
                                     jint col) {
        CursorWindow *window = GET_WINDOW(env, object);
...
// free the last row
    ...
    int register_android_database_CursorWindow(JNIEnv *env) {
        ...
        clazz = env->FindClass("org/spatialite/CursorWindow");
        if (clazz == NULL) {
            LOGE("Can't find org/spatialite/CursorWindow");
            return -1;
        }
