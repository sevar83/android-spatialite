...
int register_android_database_SQLiteProgram(JNIEnv * env)
{
    ...
    clazz = env->FindClass("org/spatialite/database/SQLiteProgram");
    if (clazz == NULL) {
        LOGE("Can't find org/spatialite/database/SQLiteProgram");
        return -1;
    }
    ...
    return jniRegisterNativeMethods(env,
        "org/spatialite/database/SQLiteProgram", sMethods, NELEM(sMethods));
}
