/* 
/ check_excel2003_biff8.c
/
/ Test cases for Excel BIFF8 format files
/
...
int main (int argc, char *argv[])
{
    ...
    ret = freexl_open ("testdata/simple2003.xls", &handle);
    if (ret != FREEXL_OK) {
    ...
    if (strcmp(worksheet_name, "simple2003_4") != 0) {
	...
	fprintf(stderr, "Unexpected cell (3,4) value: %g\n", cell_value.value.double_value);
	return -49;
    }
    ...
    if (cell_value.value.double_value != 237) {
	fprintf(stderr, "Unexpected cell (3,5) value: %g\n", cell_value.value.double_value);
	return -52;
    }
    ...
    /* The second worksheet, zero index */
    ret = freexl_get_worksheet_name(handle, 1, &worksheet_name);
    if (ret != FREEXL_OK) {
	fprintf(stderr, "Error getting worksheet name: %d\n", ret);
	return -57;
    }
    if (strcmp(worksheet_name, "sheet2") != 0) {
	fprintf(stderr, "Unexpected worksheet name: %s\n", worksheet_name);
	return -58;
    }    
    ...
    ret = freexl_get_cell_value (handle, 0, 0, &cell_value);
    if (ret != FREEXL_OK) {
	fprintf(stderr, "Error getting cell value (0,0) 2: %d\n", ret);
	return -64;
    }
    if (cell_value.type != FREEXL_CELL_SST_TEXT) {
	fprintf(stderr, "Unexpected cell (0,0) type 2: %u\n", cell_value.type);
	return -65;
    }
    if (strcmp(cell_value.value.text_value, "Column 1") != 0) {
	fprintf(stderr, "Unexpected cell (0,0) value: %s\n", cell_value.value.text_value);
    ...
    ret = freexl_get_cell_value(handle, 2, 5, &cell_value);
    if (ret != FREEXL_OK) {
	fprintf(stderr, "Error getting cell value (2,5): %d\n", ret);
	return -67;
    }
    if (cell_value.type != FREEXL_CELL_SST_TEXT) {
	fprintf(stderr, "Unexpected cell (2,5) type: %u\n", cell_value.type);
	return -68;
    }
    if (strcmp(cell_value.value.text_value, "sheet 2") != 0) {
	fprintf(stderr, "Unexpected cell (2,5) value: %s\n", cell_value.value.text_value);
	return -69;
    }
